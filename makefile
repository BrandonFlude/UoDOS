...
USERPROGS = init.exe sh.exe echo.exe ls.exe
...
# Note: I had to change this to use Q to work as I use a VM and Z is the VM itself
...
#	Mount hard image file as Q:
	imdisk -a -t file -f $(IMAGE).img -o rem -m q:
#	Now copy files to Q: (we do it this way to avoid problems with cygwin and drive specifiers)
	cmd /c "copy kernel.sys q:KERNEL.SYS"
	cmd /c "mkdir q:\usrbin"
	cmd /c "mkdir q:\usrbin\brandon"
	cmd /c "mkdir q:\usrbin\brandon\test"
#	Copy all of the user programs to Q:\usrbin
	$(foreach var,$(USERPROGS), cmd /c "copy $(var) q:\usrbin\$(var)";)	

# 	Copy the LS tool to the root directory (for Stage 4)
	cmd /c "copy ls.exe q:\ls.exe"
